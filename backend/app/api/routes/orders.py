...
async def create_order(payload: OrderCreate, request: Request):
    ...
    request_payload["ip_address"] = request.client.host if request.client else None
    request_payload["user_agent"] = request.headers.get("user-agent")

    async with AsyncSessionLocal() as session:
      result = await session.execute(
          text("""
          INSERT INTO orders (order_number, customer_name, phone_raw, phone_e164, currency, total, event_id, payload)
          VALUES (:order_number, :customer_name, :phone_raw, :phone_e164, :currency, :total, :event_id, CAST(:payload AS JSONB))
          RETURNING id
          """),
          {
              ...
              "payload": __import__("json").dumps(request_payload),
          },
      )
    ...
    return {"ok": True, "order_id": str(order_id), "order_number": order_number}
