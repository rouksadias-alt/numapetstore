...
export function ThankYouContent() {
  ...
  const whatsappHref = whatsappNumber
    ? `https://wa.me/${whatsappNumber.replace(/[^0-9]/g, "")}?text=${encodeURIComponent(
        "Hola! Tengo una pregunta sobre mi pedido en Numapetstore."
      )}`
    : null;
  ...
  return (
    <main className="grid min-h-[70vh] place-items-center bg-[#f7f2e8] px-5 py-16 text-center text-slate-950">
      <div className="max-w-2xl rounded-[2rem] bg-white p-8">
        ...
        {/* Order Summary */}
        {lines.length > 0 && (
          <div className="mt-8 rounded-2xl bg-[#f7f2e8] p-6 text-left">
            <h2 className="mb-4 text-lg font-black text-[#2a1620]">Resumen de tu pedido</h2>
            <div className="space-y-3">
              {lines.map((line) => (
                <div key={line.index} className="flex items-start justify-between gap-4">
                  <div>
                    <p className="font-bold text-[#2a1620]">{line.product.name}</p>
                    <p className="text-sm text-[#7b5867]">
                      {line.variant.label} · {line.variant.size}
                    </p>
                  </div>
                  <p className="font-black text-[#b4155a]">${line.total}</p>
                </div>
              ))}
            </div>
            <div className="mt-4 border-t border-[#ead3dd] pt-4">
              <div className="flex items-center justify-between">
                <span className="font-black text-[#2a1620]">Total</span>
                <span className="text-2xl font-black text-[#b4155a]">${total}</span>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-center">
          ...
          <Link
            href="/collection"
            className="inline-flex rounded-full bg-teal-700 px-7 py-4 font-black text-white transition hover:bg-teal-800"
          >
            Ver mas productos
          </Link>
        </div>
        ...
        {/* Trust Info */}
        <div className="mt-8 grid grid-cols-2 gap-3 text-center text-xs font-bold text-[#7b5867] sm:grid-cols-4">
          <div>
